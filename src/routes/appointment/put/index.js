...
WITH
  ...
  sub_servs_add AS (
    ...
    FROM unnest(ARRAY$2::int[]) AS recived_ss(id)
    CROSS JOIN appo
  ...
  sub_servs_del AS (
    SELECT "subServiceId"
    FROM unnest(ARRAY$3::int[]) AS recived_ss(id)
    JOIN appo_sub_servs
      ON
  ...
  end_of_available_time AS (
    SELECT
      COALESCE(appo_sub_servs_update*
        (
  ...
  FROM unnest(ARRAY$2::int[]) AS recived_ss(rid)
  JOIN sub_servs_add
  ON
  ...
  mins_to_del AS (
    SELECT COALESCE(SUM(mins), 0) AS value
    FROM unnest(ARRAY$3::int[]) AS recived_ss(rid)
    JOIN sub_services ss
    ON
      (SELECT ok FROM validations)
  ...
  remove_sub_servs AS (
    ...
    WHERE
      ...
      AND "appointmentId" = $1
      AND "subServiceId" = ANY(ARRAY$3::int[])
  ),
  join_sub_servs AS (
